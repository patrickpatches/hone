...
|---|---|---|
| #136 | `15e01df` | **EAS build DISPATCHED off main — turns #134 + #135 into an installable APK.** Patrick was stuck on build 132; the six #134 recipe-screen fixes (`e3cb60c`) and the #135 Maestro harness were committed but never built. `versionCode` bumped 50→51 so it installs cleanly over 132. Profile: preview. Dispatched by COO (build-trigger lifted by Patrick 2026-05-31). Contains HONE-007/008/010/011/012 fixes (HONE-009 still flagged to cook), HONE-016 testIDs. Per R-015: not self-closing — Patrick installs the Actions APK artifact and validates the six fixes on-device. |
