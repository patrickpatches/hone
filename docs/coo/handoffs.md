...
|---|---|---|
| #172 | `b0f04701` | **#63 + #64** — hardcoded hex → design tokens (`danger`/`dangerDeep` in both themes); `Promise.allSettled` in pantry initial load for partial-failure resilience. versionCode 59. |
| #168 | `454bd4ec` | **REPAIR — Fix truncated `seed-recipes.ts` from build #167 (fixes #55 + #56 reaching device).** Build #167 failed with `SyntaxError: Unterminated string constant` in Hermes bundling — the Cook's recipe fix commit was written to disk truncated mid-string. Fix: spliced the Cook's valid content (lines 1–5831) onto the complete tail from the last-known-good file (`ce3ba55`). Cook's intended fixes confirmed present: kasuri methi in butter chicken (#55) and Pavlova amounts aligned to Donna Hay (#56). Also added `hard_to_find` + `local_alternative` fields to the `Ingredient` schema (first use: kasuri methi; consistent with Golden Rule #3). CI validation gate added (`6999686`): `eas-build.yml` now gates the APK job on a `validate` job that runs hermes-parser + R-014 checks — truncated file aborts in ~1 min not ~8. versionCode 57 (unchanged). EAS preview. Per R-015: not self-closing. |
| #167 | `1b1d13b1` | **Cook recipe data fixes — Butter Chicken #55 + Pavlova #56.** Recipe data only (EAS preview). #55 (Serious): added kasuri methi (dried fenugreek leaves, 1 tbsp) as ingredient i21 — this is the defining makhani ingredient the credited Joshua Weissman method uses; includes hard_to_find flag + local_alternative (Indian grocers / some Woolworths-Coles). Step s5 updated to instruct crushing and stirring it in during final cream simmer. Garam masala (i7, 2 tsp) split correctly: 1 tsp marinade + 1 tsp sauce; step s3 'remaining garam masala' text fixed; mise_en_place updated to match. #56 (Tidy-up): cornflour 2 tsp → 1 tbsp and white wine vinegar 1 tsp → 1.5 tsp to match credited Donna Hay amounts. versionCode 56 → 57. EAS preview dispatched. Per R-015: not self-closing. |
| #166 | `ce3ba55` | **Fix cross-tab bleed-through: each tab scene paints its own backdrop.** Root cause: React Navigation bottom-tabs keeps inactive scenes mounted, relying on the active scene being opaque. Screens were transparent for the shared illustration, so Kitchen bled through behind Pantry and Shop. Fix: each tab scene gets its own opaque backdrop via React Navigation v7 `screenLayout` wrapper — dark → solid `tokens.bg` View; light native → ImageBackground; light web → View with CSS backgroundImage. `sceneContainerStyle`/`sceneStyle: transparent` corrected; redundant root-level ImageBackground removed; `expo-asset` added as a top-level dep. versionCode 57 (unchanged). EAS preview. Per R-015: not self-closing. |
| #165 | `716dd18d` | **Guarantee illustration on all tab screens (native Android).** `sceneContainerStyle: transparent` on bottom-tabs was silently ignored (Stack-only prop). Fix: `ImageBackground` placed directly inside `(tabs)/_layout.tsx` as the immediate Tabs parent — no reliance on navigation container transparency. Light native: Tabs in ImageBackground; dark native / web: Tabs returned directly (AppShell covers). AppShell root ImageBackground kept for non-tab screens. versionCode 57 (unchanged). EAS preview. Per R-015: not self-closing. |
| #164 | `804a7488` | **Fix Sydney Harbour Bridge illustration not showing on tab screens (native Android).** Root cause: Tabs navigator had no `contentStyle`, so Android defaulted tab content to white — blocking the ImageBackground on every tab screen. Fix: (1) `contentStyle: { backgroundColor: 'transparent' }` added to Tabs `screenOptions`; (2) web branch hardcodes `'#C20060'` (tokens.bg is now `'transparent'` for native); (3) `global.css` kills Expo's default grey html/body/#root background on web. Removed unused `LinearGradient` imports + gradient constants. versionCode 57 (unchanged). EAS preview. Per R-015: not self-closing. |
| #163 | `9d32f494` | **feat(light-theme): Sydney Harbour Bridge synthwave background.** Light mode now uses a Sydney Harbour Bridge photo as the full-screen app background image, replacing the solid magenta `#C20060`. Dark mode unchanged. versionCode 57 (unchanged). EAS preview. Per R-015: not self-closing. |
| #162 | `5c77e778` | **Cook recipe data fixes — Weekday Bolognese #53 + Chicken Shawarma #54.** Recipe data only (EAS preview). #53: source credit changed from Andy Cooks (no verified video) → Tucker & Spice Kitchen after Marcella Hazan (*Essentials of Classic Italian Cooking*, 1992) — milk-first method is Hazan's, not Andy's; `total_time_minutes` + `time_min` corrected 75 → 85 min. #54: thigh doneness target corrected from 74 °C (safety floor) → 80–85 °C per ThermoWorks + America's Test Kitchen (collagen dissolves higher; breast sub temp untouched); 'grill / broil' → 'grill' everywhere in the recipe (AU English). versionCode 55 → 56. EAS preview dispatched. Per R-015: not self-closing. |
